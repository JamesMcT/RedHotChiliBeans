...
/**
 * The data import service. This class is responsible for watching a folder on
 * the server for new files. It will also take delivery of a file from a client
 * directly (e.g. a rest based service). This class will, at startup, begin
 * watching a specified directory for new files.
 * 
 * @author Eoin Kernan
 *
 */
...
public class DataImportService implements DataImportServiceLocal {
	...
	// Responsible for interacting with DAO objects and persisting business
	// entities through same.
	@EJB
	...
	// choose which folder path to watch, based on underlying OS. If not
	// Windows, then Linux is assumed.
	private final static String WATCH_PATH = System.getProperty("os.name")
			.startsWith("Windows") ? "c:/watching/" : "/watching/";

	private final static String PROCESSED_FILE_SUFFIX = ".processed";

	// A map of maps, one map for each cached entity type, using entity name as
	// key.
	...
	private void initialiseHashMaps() {
		...
		entityMap.put(EventCauseReader.getName(),
				new HashMap<EventCausePK, EventCause>());
		entityMap.put(FailureTypeReader.getName(),
				new HashMap<Integer, FailureType>());
		entityMap.put(UserEquipmentReader.getName(),
				new HashMap<Integer, UserEquipment>());
		entityMap.put(OperatorCountryReader.getName(),
				new HashMap<OperatorCountryPK, OperatorCountry>());
	...
	/**
	 *
	 * Given that this class is a singleton, and this method is synchronized,
	 * only one file can be processed at any given time. This will stop multiple
	 * clients from attempting to upload data simultaneously. Upload/input is
	 * not thought to be a very frequent activity, so this restriction should
	...
	/**
	 * This method will spin off a thread to watch the folder that is supplied
	 * in the folderPath String.
	 * 
	 * @param folderPath
	 */
	public void startDirectoryWatcher(final String folderPath) {
		...
		logger.info(String.format("startDirectoryWatcher() watching...",
				folderPath));

		try {
			final WatchService directoryWatcher = FileSystems.getDefault()
					.newWatchService();

			// watchDirectory() contains "directoryWatcher.take()", which is a
			// blocking method
	...
	private void watchDirectory(WatchService directoryWatcher, String folderPath) {
		...
		for (;;) {
			...
			for (WatchEvent<?> event : key.pollEvents()) {
				...
				// Verify that the new file is an Excel file, restart the
				// watcher loop if not
				try {
					if (!Files.probeContentType(pathname).equals(
							"application/vnd.ms-excel")
							&& !Files
									.probeContentType(pathname)
									.equals("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")) {
						logger.error(String.format(
								"New file '%s' is not a spreadsheet file.",
								filename));
						continue;
					}
				} catch (IOException ioe) {
	 ...
	 * 
	 * @param URI
	 *            A URI pointing to the file to be used to initialise the
	 *            workBook,
	 */
	public void initialiseWorkBook(String URI) {
		try {
			workBook = new HSSFWorkbook(new BufferedInputStream(
					new FileInputStream(URI), 1024 * 1024));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void processWorkBook() {
		try {
			...
			logger.info("Input processing complete in "
					+ new DecimalFormat("0.00").format(timeTaken) + "s");

			// Close workBook when finished
			workBook.close();

		} catch (IOException e) {
			// File exception...maybe file is locked. Watcher will continue
			// watching the folder
			e.printStackTrace();
	...
	private void renameFileAfterProcessing(String fullName) {
		...
		// If new name exists, keep appending an (int) until you reach one that
		// doesn't exist yet.
		int failCount = 1;
		while (file2.exists()) {
		...
		if (file.renameTo(file2)) {
			logger.info(String.format("Successfully renamed file '%s' to '%s'",
					fullName, finalName));
		} else {
			logger.info(String.format("Error renaming '%s' to '%s'", fullName,
					finalName));
		}
	}
	...
	private void incrementFileCount() {
		logger.info(String.format("Incrementing fileCount from %d to %d",
