function readDates($scope, $http) {
	...
	$scope.sayHello = function(firstDate, secondDate) {
		...
		if (!validateDate(firstDate, "Invalid start date")
				|| !validateDate(secondDate, "Invalid end date")) {
...
function validateDate(dateString, errorMessage) {
	...
	if (dateString.match(regexPattern)) {
		console.log("pass");
		// document.getElementById('dataTables-example').innerHTML = "<div
		// ng-controller='readDates'>Invalid date1</div>";
